...
class WebSocket
{
    ...
    /**
     * Get jwt token from cookie in headers.
     * @param string $cookie_string
     * @return bool|mixed|string
     */

    private function getJWTToken($cookie_string)
    {
        if ($cookie_string) {
            $cookie_string = str_replace(' ', '', $cookie_string);
            $cookies_arr = preg_split("/;/", $cookie_string); //extract each cookie "key=val" into each index
            foreach ($cookies_arr as $cookie) { //extract session cookie from received headers
                $cookie_key_val = explode('=', $cookie);
                if ($cookie_key_val[0] == 'jwt') {
                    return $cookie_key_val[1];
                }
            }
        }
        return false;
    ...
    /**
     * Set userid of client from jwt cookie.
     * @param Client $client
     * @return bool
     */

    private function setUserID($client)
    {
        if ($token = $this->getJWTToken($client->getHeader('Cookie'))) {
            if ($user_info = json_decode($this->jwt->get_user_details_from_jwt($token), true)) {
                if (!empty($user_info['user_id'])) {
                    $client->setUserID($user_info['user_id']);
                    return true;
                } else {
    ...
    public function loop()
    {
        while (is_resource($this->socket)) {
            ...
            $write = [];
            $except = [];
            if ($this->select($read, $write, $except, 0, 1) === false) {
                $this->onError($this->socket);
            }
...
//                            $this->setUserID($client);
                            if (!$this->setUserID($client)) {
                                ...
                                $this->log("Client is disconnected", $client);
                                $this->disconnectClient($client, self::STATUS_CLOSE_NO_STATUS, "Invalid Cookie");
                                continue;
                            }
                            $this->clients[] = $client;
...
//                        $this->log("Receive data: " . $data['payload'], $client);
     ...
     * @param bool $masked
     */
    public function sendData(Client $client, $data, $masked = false)
    {
